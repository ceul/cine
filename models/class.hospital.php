...
function select($id){
	$sql =  "SELECT * FROM hospital.estado_habitacion WHERE id_estado = '$id'";
	try {
		$row = pg::query($sql);
...
function delete($id){
	$sql = "DELETE FROM hospital.estado_habitacion WHERE id_estado = '$id'";
	try {
		pg::query("begin");
		$row = pg::query($sql);
		pg::query("commit");
...
//echo "me llamoooooo";
	if ($this->validaP($this->id) == false){
		$sql = "INSERT INTO hospital.estado_habitacion( id_estado, nombre, descripcion) VALUES ( '$this->id', '$this->nombre', '$this->descripcion')";
		try {
			pg::query("begin");
			$row = pg::query($sql);
			pg::query("commit");
		...
		$sql="UPDATE hospital.estado_habitacion set nombre='" . $this->nombre . "' WHERE id_estado='" . $this->id . "'";
		pg::query("begin");
		$row = pg::query($sql);
		pg::query("commit");		
		echo "2";
...
function getTabla(){
	...
	try {
		...
		while ($row= pg_fetch_array($result)){
			...
			echo "	<th>" . $row['id_estado'] . "</th>";
			echo "	<th>" . $row['nombre'] . "</th>";
                        echo "	<th>" . $row['descripcion'] . "</th>";
			echo "	<th><a href='#' class='btn btn-danger' onclick='elimina(\"" . $row['id_estado'] . "\")'>X<i class='icon-white icon-trash'></i></a>.<a href='#' class='btn btn-primary' onclick='edit(\"" . $row['id_estado'] . "\", \"" . $row['nombre'] . "\", \"" . $row['descripcion'] . "\")'>E<i class='icon-white icon-refresh'></i></a></th>";
			echo "</tr>";
		}
		echo "</tbody>";
		echo "</table>";
